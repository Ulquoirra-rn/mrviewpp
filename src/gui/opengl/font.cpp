    ...
    namespace GL
    {
      ...
      void Font::initGL (bool with_shadow)
      {
        ...
        for (int c = first_char; c <= last_char; ++c)
          ...
          #else
          tex_width += metric.horizontalAdvance (QChar(uchar(c))) + 2;
          #endif
        ...
        for (int c = first_char; c <= last_char; ++c) {
          pixmap.fill (0);
          painter.drawText (1, metric.ascent() + 1, QString(QChar(uchar(c))));
          #if QT_VERSION < QT_VERSION_CHECK(5, 11, 0)
          font_width[c] = metric.width (c);
          #else
          font_width[c] = metric.horizontalAdvance (QChar(uchar(c)));
          #endif
