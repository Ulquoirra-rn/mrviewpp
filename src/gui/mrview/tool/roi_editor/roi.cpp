  ...
  namespace GUI
  {
    namespace MRView
    {
      namespace Tool
      {
        ...
        // Read an ROI's GPU texture back to a flat CPU buffer (x fastest).
        static void read_roi_voxels (ROI_Item* roi, vector<GLubyte>& data)
        {
          data.resize (size_t(roi->header().size(0)) * roi->header().size(1) * roi->header().size(2));
        ...
        void ROI::write_roi_mask (ROI_Item* roi, const std::string& path)
        {
          vector<GLubyte> data;
        ...
        {
          const size_t nx = rois[0]->header().size(0);
          const size_t ny = rois[0]->header().size(1);
          const size_t nz = rois[0]->header().size(2);
          vector<uint32_t> label (nx * ny * nz, 0);
          {
            vector<GLubyte> data;
            for (size_t i = 0; i != rois.size(); ++i) {
              read_roi_voxels (rois[i], data);
              const size_t n = std::min (label.size(), data.size());
              for (size_t v = 0; v != n; ++v)
                if (data[v]) label[v] = uint32_t (i + 1);   // later ROI wins on overlap
            }
          }

          MR::Header header (rois[0]->header());
          header.ndim() = 3;
          header.datatype() = (rois.size() < 65535) ? DataType::UInt16 : DataType::UInt32;
          header.datatype().set_byte_order_native();
          auto out = MR::Image<uint32_t>::create (path, header);
          for (auto l = MR::Loop(out) (out); l; ++l)
            out.value() = label[out.index(0) + nx * (out.index(1) + ny * out.index(2))];

          // Write an LUT sidecar mapping each label value to its ROI name/colour.
          std::string stem = path;
          const size_t dot = stem.find_last_of ('.');
          if (dot != std::string::npos) {
            std::string s = stem.substr (0, dot);
            const size_t dot2 = s.find_last_of ('.');
            if (stem.substr (dot) == ".gz" && dot2 != std::string::npos) s = s.substr (0, dot2);
            stem = s;
          }
          const std::string lut = stem + "_lut.txt";
          File::OFStream out_lut (lut);
          out_lut << "# Label lookup table generated by mrview++\n";
          out_lut << "# index name red green blue alpha\n";
          out_lut << "0 background 0 0 0 0\n";
          for (size_t i = 0; i != rois.size(); ++i) {
            std::string name = Path::basename (rois[i]->get_filename());
            for (auto& c : name) if (c == ' ') c = '_';
            out_lut << (i + 1) << " " << name << " "
                    << int(rois[i]->colour[0]) << " " << int(rois[i]->colour[1]) << " "
                    << int(rois[i]->colour[2]) << " 255\n";
          }
        }
        ...
        {
          QModelIndexList indices = list_view->selectionModel()->selectedIndexes();
          vector<ROI_Item*> rois;
          for (QModelIndex idx : indices)
            if (ROI_Item* r = dynamic_cast<ROI_Item*> (list_model->get (idx)))
              rois.push_back (r);
          if (rois.empty())
            return;
          ...
          try {
            if (choice.mode == Dialog::File::MultiSaveMode::SingleFile) {
              ...
              for (ROI_Item* r : rois) r->saved = true;
            }
            else {
              std::string folder = Dialog::File::get_folder (this, "Select folder for exported ROIs", &current_folder);
              if (folder.empty())
                return;
              for (ROI_Item* r : rois) {
        ...
        void ROI::close_slot ()
        {
          // Close every selected ROI (prompting to save any modified ones).
          QModelIndexList indices = list_view->selectionModel()->selectedIndexes();
          if (indices.empty())
            return;
          ...
          for (QModelIndex idx : indices)
            if (ROI_Item* r = dynamic_cast<ROI_Item*> (list_model->get (idx)))
              if (!r->saved) { any_unsaved = true; break; }

          if (any_unsaved) {
            size_t ret = QMessageBox::warning (this, tr("ROI not saved"),
                tr("One or more selected ROIs have been modified. Do you want to save them?"),
          ...
          while (true) {
            QModelIndexList sel = list_view->selectionModel()->selectedIndexes();
            if (sel.empty())
              break;
